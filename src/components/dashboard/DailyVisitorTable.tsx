    ...
    <div className={`glass glass-hover noise rounded-3xl p-6 overflow-hidden animate-fade-in ${className}`}>
      ...
      <div className="overflow-x-auto scrollbar-none">
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-[1fr_1fr_1fr] gap-0 px-4 pb-3 text-[10px] font-semibold tracking-wider uppercase text-white/30">
              <div>Date</div>
              <div>PV</div>
              <div>UV</div>
            </div>
            <div className="overflow-hidden rounded-[1.75rem] border border-white/[0.08]">
              {current.map((r) => (
                <div
                  key={r.id ?? r.date}
                  className="grid grid-cols-[1fr_1fr_1fr] gap-0 px-4 py-3 border-b border-white/[0.08] last:border-b-0 transition-colors bg-white/[0.025] hover:bg-white/[0.04]"
                >
                  ...
                  <div className="text-xs text-white/70 self-center tabular-nums font-medium">
                    {r.pv.toLocaleString()}
                  </div>
                  <div className="text-xs text-white/70 self-center tabular-nums font-medium">
                    {r.uv.toLocaleString()}
