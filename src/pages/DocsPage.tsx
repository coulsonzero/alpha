...
const loadMd = (path: string, original: string): string => {
  try { return localStorage.getItem(`docs-md:${path}`) ?? original; } catch { return original; }
};
const saveMd = (path: string, md: string) => {
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex rounded-xl p-0.5 gap-0.5" style={{ background: "rgba(255,255,255,0.04)",border: "1px solid rgba(255,255,255,0.05)" }}>
              <button
                onClick={() => { if (isEditing) { const p = sel?.path; if (p) saveMd(p, editContent); setArticleMd(editContent); setIsEditing(false); if (selectedIdx !== null) ARTICLES[selectedIdx] = { ...ARTICLES[selectedIdx], md: editContent }; } else if (editContent) setArticleMd(editContent); setViewMode("preview"); }}
            ...
            {isEditing && (
              <button
                onClick={() => { const p = sel?.path; if (p) saveMd(p, editContent); setArticleMd(editContent); setIsEditing(false); setViewMode("preview"); if (selectedIdx !== null) ARTICLES[selectedIdx] = { ...ARTICLES[selectedIdx], md: editContent }; }}
